...
def create_CSS_Tenure(type, figure_count, colours, partial_output_path, MI_folder_path, data_label_font_dict_white, data_label_font_dict_black):
    ###########
    # Main script Notifications
    if type==0:
        print(f'Figure{figure_count}_CSS_Tenure')

    if type==1:
        print(f'Accessible_Figure{figure_count}_CSS_Tenure')
    ###########
    ...
    CSS_4 = chop_df(CSS_4, 3, 3)

    # Select the required columns
    number_of_social = CSS_4.iloc[:, 1].reset_index(drop=True)
    number_of_private = CSS_4.iloc[:, 3].reset_index(drop=True)

    max_total = max(sum(number_of_social), sum(number_of_private))

    data = pd.DataFrame({
        "Remediation complete": [number_of_social[0],number_of_private[0]],
        "Works started": [number_of_social[1], number_of_private[1]],
        "In programme": [number_of_social[2], number_of_private[2]]
    }, index=["Social", "Private"])
