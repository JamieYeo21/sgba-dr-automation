...
def create_Developer_Height(type, figure_count, colours, partial_output_path, MI_folder_path, data_label_font_dict_white, data_label_font_dict_black):
    ###########
    # Main script Notifications
    if type==0:
        print(f'Figure{figure_count}_Developer_Height')

    if type==1:
        print(f'Accessible_Figure{figure_count}_Developer_Height')
    ###########
    ...
    number_of_11_18m = Developer_1a.iloc[:, 1].reset_index(drop=True)
    number_of_18m = Developer_1a.iloc[:, 3].reset_index(drop=True)
    ...
    data = pd.DataFrame({
        ...
        "Remediation started": [number_of_11_18m[2], number_of_18m[2]],
        "Plans in place": [number_of_11_18m[3], number_of_18m[3]],
        "No plans in place": [number_of_11_18m[4], number_of_18m[4]],
    }, index=["11-18m", "18m+"])
