...
def create_Overall_Height(type, figure_count, colours, parital_output_path, data_label_font_dict_white, data_label_font_dict_black):
    ###########
    # Main script Notifications
    if type==0:
        text = f'Figure{figure_count}_Overall_Height'
        print(text)

    if type==1:
        text = f'Accessible_Figure{figure_count}_Overall_Height'
        print(text)
    ###########
    ...
    number_of_11_18m = Combined_2.iloc[:, 1].reset_index(drop=True)
    number_of_18m = Combined_2.iloc[:, 3].reset_index(drop=True)

    max_total = max(sum(number_of_11_18m), sum(number_of_18m))

    data = pd.DataFrame({
        "Remediation complete": [number_of_11_18m[0],number_of_18m[0]],
        "Remediation underway": [number_of_11_18m[1], number_of_18m[1]],
