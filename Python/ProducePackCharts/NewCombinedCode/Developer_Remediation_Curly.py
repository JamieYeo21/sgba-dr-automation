...
def create_Developer_Remediation_Curly(type, figure_count, colours, grey, partial_output_path, MI_folder_path, data_label_font_dict_white, data_label_font_dict_black, brace_label_font_dict):
    ###########
    # Main script Notifications
    if type==0:
        print(f'Figure{figure_count}_Developer_Remediation_Curly')

    if type==1:
        print(f'Accessible_Figure{figure_count}_Developer_Remediation_Curly')
    ###########
    ...
    number_of_buildings = Developer_1a.iloc[:, 5].reset_index(drop=True)
